...
GRADLE_FILES = [ROOT / "app" / "build.gradle", ROOT / "ktheme-kotlin" / "build.gradle"]
OSV_QUERY = "https://api.osv.dev/v1/query"

STRING_DEP_RE = re.compile(r"(?:implementation|api|compileOnly|runtimeOnly|testImplementation)\s+['\"]([^'\"]+)['\"]")
ALIAS_DEP_RE = re.compile(r"(?:implementation|api|compileOnly|runtimeOnly|testImplementation)\s+libs\.([A-Za-z0-9_.]+)")
        ...
        if section == "versions":
            ...
            if m:
                ...
                if version_ref and version_ref in versions:
                    libs[alias.replace("-", ".")] = f"{module}:{versions[version_ref]}"
                else:
                    libs[alias.replace("-", ".")] = module
